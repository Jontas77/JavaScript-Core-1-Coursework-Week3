...
*/
function getTransportModes(type) {
  return type.slice(1, 3);
}
...
*/
function isAccessibleByTransportMode(ride1, ride2) {
  if(ride1 === ride2) {
    return true;
  } else {
    return false;
...
function journeyPlanner(locations, transportMode) {
  return locations.filter(ride => ride.slice(1).includes(transportMode)).map(place => place[0]);
}
